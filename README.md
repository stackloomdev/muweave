...
`pnpm start` 只提供静态文件。Vercel 可使用仓库中的 `vercel.json`：构建命令 `pnpm build`，发布目录 `apps/studio/dist`。独立网页也可只执行 `pnpm build:studio`，将该目录发布到静态 HTTPS 托管。ChatGPT 接入另包含可选的 `/api/mcp` 函数，详情见下文。
...
新增无状态远程 MCP 与聊天画布：创建文字/形状分镜、按完整快照修改、校验、预览并下载工程包。工程包在幕织导入后继续添加素材和导出视频。运行 `pnpm dev:mcp` 启动开发入口；部署、插件包与数据隔离说明见 [ChatGPT 接入指南](docs/CHATGPT.md)。

远程工具只处理本次提交的快照，不保存用户工程，不读取浏览器素材，不提供按工程 ID 查询数据。它与页面上的原生 WebMCP 是两个不同入口。

## 数据与限制

工程和素材保存在**当前浏览器、当前站点来源**的 IndexedDB 中。换域名、端口、浏览器或设备不会自动同步。清除站点数据会删除其中的工程；请通过工程包备份和迁移。网页素材不上传服务器；使用远程 MCP 时，明确提交的分镜文字/布局会经过 MCP 服务器和宿主。生成图片和配音的模型仍在宿主侧，生成结果通过浏览器文件选择器导入。
